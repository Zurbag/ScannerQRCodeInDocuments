...
public class CvUtils {
    ...
    //Ну из названия вдно что это просмотри изображения
    ...
    //Просмотр изображения только матрица внутри собирается
    public static void showImageTo(String fileName) {
        Mat img = Imgcodecs.imread(fileName);
        if (img.empty()) {
            System.out.println("Не удалось загрузить изображение");
            return;
        }
        CvUtils.showImage(img, fileName);//Второй параметр это текст в окне заголовка
    }
}
